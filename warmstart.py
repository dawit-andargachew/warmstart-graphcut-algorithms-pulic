...
def FeasRestoreIter(p, rGraph, excesses, s, t):
  def FindPath(chain, node, excesses, s, t, out_dir=True):
    ...
    while u >= 0:
      ...
      v = u
      u = chain[v]
    if v not in [s, t]: path_flow = min(path_flow, abs(excesses[v]))
    if out_dir: path.reverse()
    return path, path_flow


  if excesses[p] == 0: return [p], 0
  direc = (excesses[p] > 0)
  ...
  if t in visited: return FindPath(chain, t, excesses, s, t, direc)
  if s in visited: return FindPath(chain, s, excesses, s, t, direc)
...
def FeasProj(flows, graph, excesses, V, s, t):
  ...
  begin = time.time()
  for i in range(V):
    if i in [s, t] or excesses[i] == 0: continue
    while excesses[i] != 0:
      path, pathFlow = FeasRestoreIter(i, rGraph, excesses, s, t)
      if pathFlow is None:
        raise Exception("Code has a bug. Didn't find a path during feasibility projection.")
...
def WarmStartFlow(inf_flows, graph, excesses, V, s, t):
  begin = time.time()
  proj_flows, rGraph, proj_time, num_paths_proj, avg_len_proj = FeasProj(inf_flows, graph, excesses, V, s, t)
  proj_value = sum([proj_flows[s][v] for v in proj_flows[s]])
  max_flows, cuts, num_paths_aug, avg_len_aug = augmentingPath(graph, V, s, t, proj_flows, rGraph)
  end = time.time()
  print("warmstart total time: ", end - begin)
  print("finding maximum flow # of augmenting path: %d, average augmenting path length: %f" % (num_paths_aug, avg_len_aug))
...
def Exp(args, trial):
  ...
  time_dir = result_dir + str(size) + '_time_{}.txt'.format(str(trial))
  path_dir = result_dir + str(size) + '_path_{}.txt'.format(str(trial))

  time_f = open(time_dir, 'w')
  time_f.write("image_name\tff\twarm_start\tfeas_proj\n")
  path_f = open(path_dir, 'w')
  path_f.write("image_name\tflow_value\texcess\trecoverd_flow\tnum_aug_path\tavg_path_len\tnum_proj_path\tavg_proj_len\tnum_warm_start_path\tavg_warm_start_len\n")

  num_images = len(image_list)
  old_flows = None
  for i in range(num_images):
    new_image = image_list[i]
    new_flows,  min_cut, path_count, average_path_len, new_graph, ff_time = imageSegmentation(new_image, folder, group, (size, size), algo, loadseed)
    if old_flows is None:
      old_flows = deepcopy(new_flows)
      continue
    excesses = RoundDown(old_flows, new_graph, V, SOURCE, SINK)
    total_excess = max(np.sum(np.maximum(excesses, 0)), np.sum(np.maximum(-excesses, 0)))
    print("total excess/deficit to round down:" + str(total_excess))
    warmstart_flows, warmstart_cuts, proj_value, proj_time, warmstart_time, num_paths_proj, avg_len_proj, num_paths_aug, avg_len_aug = WarmStartFlow(old_flows, new_graph, excesses, V, SOURCE, SINK)

    # check that the warm start algo reaches optimality
    opt_flow_value = sum([new_flows[SOURCE][v] for v in new_flows[SOURCE]])
    ws_flow_value = sum([warmstart_flows[SOURCE][v] for v in warmstart_flows[SOURCE]])
    ...
    path_f.write('{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(
      ...
      proj_value,
      path_count,
      round(average_path_len, 4),
      num_paths_proj,
      round(avg_len_proj, 4),
      num_paths_aug,
      round(avg_len_aug, 4)
    ))
    path_f.flush()
